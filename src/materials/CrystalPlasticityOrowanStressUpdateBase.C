...
InputParameters
CrystalPlasticityOrowanStressUpdateBase::validParams()
{
  ...
  // Additional parameters for discoflux model
...
CrystalPlasticityOrowanStressUpdateBase::CrystalPlasticityOrowanStressUpdateBase(
  ...
  : Material(parameters),
    ...
    _dislocation_immobile_edge_positive_old(    getMaterialPropertyOld<std::vector<Real>>(_base_name + "dislocation_immobile_edge_positive")),
    ...
    _dislocation_immobile_screw_positive_old(   getMaterialPropertyOld<std::vector<Real>>(_base_name + "dislocation_immobile_screw_positive")),
    _dislocation_immobile_screw_negative_old(   getMaterialPropertyOld<std::vector<Real>>(_base_name + "dislocation_immobile_screw_negative")),
    ...
    _dislo_velocity_edge_old(                   getMaterialPropertyOld<std::vector<Real>>(_base_name + "dislo_velocity_edge")),
    _dislo_velocity_screw(                      declareProperty<std::vector<Real>>(_base_name + "dislo_velocity_screw")),
    _dislo_velocity_screw_old(                  getMaterialPropertyOld<std::vector<Real>>(_base_name + "dislo_velocity_screw")),
...
void
CrystalPlasticityOrowanStressUpdateBase::getSlipSystems()
{
  ...
  if (orthonormal_error)
    mooseError(
        "CrystalPlasticityOrowanStressUpdateBase Error: The slip system file contains a slip "
        "direction and plane normal pair that are not orthonormal in the Cartesian "
        "coordinate system.");
}

void
CrystalPlasticityOrowanStressUpdateBase::transformHexagonalMillerBravaisSlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
  ...
  if (_unit_cell_dimension[0] != _unit_cell_dimension[1] ||
    ...
    mooseError(
        "CrystalPlasticityOrowanStressUpdateBase Error: The specified unit cell dimensions are "
        "not consistent with expectations for "
        "HCP crystal hexagonal lattices.");
  else if (reader.getData(0).size() != miller_bravais_indices * 2)
    mooseError(
        "CrystalPlasticityOrowanStressUpdateBase Error: The number of entries in the first row of "
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    for (const auto j : index_range(reader.getData(i)))
    {
      ...
      if (basal_pl_sum > _zero_tol)
        mooseError(
            "CrystalPlasticityOrowanStressUpdateBase Error: The specified HCP basal plane "
            "Miller-Bravais "
      ...
      if (basal_dir_sum > _zero_tol)
        mooseError(
            "CrystalPlasticityOrowanStressUpdateBase Error: The specified HCP slip direction "
...
void
CrystalPlasticityOrowanStressUpdateBase::sortCrossSlipFamilies()
{
...
unsigned int
CrystalPlasticityOrowanStressUpdateBase::identifyCrossSlipFamily(const unsigned int index)
{
  for (unsigned int i = 0; i < _number_cross_slip_directions; ++i)
    for (unsigned int j = 0; j < _number_cross_slip_planes; ++j)
...
void
CrystalPlasticityOrowanStressUpdateBase::calculateFlowDirection(const RankTwoTensor & crysrot)
{
...
void
CrystalPlasticityOrowanStressUpdateBase::calculateSchmidTensor(
...
void
CrystalPlasticityOrowanStressUpdateBase::calculateShearStress(
...
void
CrystalPlasticityOrowanStressUpdateBase::calculateTotalPlasticDeformationGradientDerivative(
...
void
CrystalPlasticityOrowanStressUpdateBase::calculateEquivalentSlipIncrement(
...
void
CrystalPlasticityOrowanStressUpdateBase::setQp(const unsigned int & qp)
{
  _qp = qp;
}

void
CrystalPlasticityOrowanStressUpdateBase::setSubstepDt(const Real & substep_dt)
{
  _substep_dt = substep_dt;
}

bool
CrystalPlasticityOrowanStressUpdateBase::isConstitutiveStateVariableConverged(
