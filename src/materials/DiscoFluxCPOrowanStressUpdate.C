...
DiscoFluxCPOrowanStressUpdate::validParams()
{
  ...
  params.addParam<Real>("initial_thermal", 0, "initial athermal stress resistance in MPa");
  ...
  params.addParam<Real>("Coeff_thermal", 0.0, "parameter to control the thermal stress");
...
DiscoFluxCPOrowanStressUpdate::DiscoFluxCPOrowanStressUpdate(const InputParameters & parameters)
  : CrystalPlasticityOrowanStressUpdateBase(parameters),
    ...
    _sat_coef_imm(getParam<Real>("sat_coef_imm")),
...
DiscoFluxCPOrowanStressUpdate::getDDIncrements()
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    ...
    for (unsigned int j = 0; j < _number_slip_systems; ++j)
    {
      ...
      _dislocation_forest[_qp][i] += A_f_ij * (_dislocation_mobile_screw[_qp][j] +
    ...
    if (std::abs(_slip_increment[_qp][i]) > small2)
    {
      _dislocation_immobile_increment_edge_positive[i] =
          _C_trap * std::pow(_dislocation_forest[_qp][i], 0.5) *
              std::abs(_DD_EdgePositive[i] * _dislo_velocity_edge[_qp][i]) -
          (0.25 * _C_im_ann * std::pow(_dislocation_forest[_qp][i], 0.5) /
           (_dd_sat * _sat_coef_imm)) *
              _DD_EdgePositive[i] * _DD_EdgePositive[i] * std::abs(_dislo_velocity_edge[_qp][i]);
      _dislocation_immobile_increment_edge_negative[i] =
          _C_trap * std::pow(_dislocation_forest[_qp][i], 0.5) *
              std::abs(_DD_EdgeNegative[i] * _dislo_velocity_edge[_qp][i]) -
          (0.25 * _C_im_ann * std::pow(_dislocation_forest[_qp][i], 0.5) /
           (_dd_sat * _sat_coef_imm)) *
              _DD_EdgeNegative[i] * _DD_EdgeNegative[i] * std::abs(_dislo_velocity_edge[_qp][i]);
      _dislocation_immobile_increment_screw_positive[i] =
          _C_trap * std::pow(_dislocation_forest[_qp][i], 0.5) *
              std::abs(_DD_ScrewPositive[i] * _dislo_velocity_screw[_qp][i]) -
          (0.25 * _C_im_ann * std::pow(_dislocation_forest[_qp][i], 0.5) /
           (_dd_sat * _sat_coef_imm)) *
              _DD_ScrewPositive[i] * _DD_ScrewPositive[i] * std::abs(_dislo_velocity_screw[_qp][i]);
      _dislocation_immobile_increment_screw_negative[i] =
          _C_trap * std::pow(_dislocation_forest[_qp][i], 0.5) *
              std::abs(_DD_ScrewNegative[i] * _dislo_velocity_screw[_qp][i]) -
          (0.25 * _C_im_ann * std::pow(_dislocation_forest[_qp][i], 0.5) /
           (_dd_sat * _sat_coef_imm)) *
              _DD_ScrewNegative[i] * _DD_ScrewNegative[i] * std::abs(_dislo_velocity_screw[_qp][i]);
    }
    else
    {
...
DiscoFluxCPOrowanStressUpdate::updateStateVariables()
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    ...
    _dislocation_immobile[_qp][i] =
        _dislocation_immobile_edge_positive[_qp][i] + _dislocation_immobile_edge_negative[_qp][i] +
...
DiscoFluxCPOrowanStressUpdate::areConstitutiveStateVariablesConverged()
{
  ...
  //                                          _resistance_tol);
  flagSlipResistanceConverged = true;
...
DiscoFluxCPOrowanStressUpdate::DDCUpdate()
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    ...
    for (unsigned int j = 0; j < _number_slip_systems; ++j)
    {
      ...
      // _slip_plane_normalboth[_qp][j].cross(_slip_direction_edge[_qp][j])));
      theta =
          std::acos(_slip_plane_normalboth[_qp][i] *
                    (_slip_plane_normalboth[_qp][j].cross(_slip_direction_edge[_qp][j])) /
                    (_slip_plane_normalboth[_qp][i].norm() *
                     _slip_plane_normalboth[_qp][j].cross(_slip_direction_edge[_qp][j]).norm()));

      A_c_ij = 0.5 * std::abs(std::sin(theta));

      _dislocation_coplanar[_qp][i] +=
          A_c_ij * (_dislocation_mobile_edge[_qp][j] + _dislocation_immobile_edge_positive[_qp][j] +
                    _dislocation_immobile_edge_negative[_qp][j]);
      // Screw contribution
      // A_c_ij = 0.5 *
      // std::abs(_slip_plane_normalboth[_qp][i].cross(_slip_direction_edge[_qp][j]));
      theta =
          std::acos(_slip_plane_normalboth[_qp][i] * (_slip_direction_edge[_qp][j]) /
                    (_slip_plane_normalboth[_qp][i].norm() * _slip_direction_edge[_qp][j].norm()));
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    slip_direction_rotated = _slip_direction_edge[_qp][i];      //_slip_direction[i];
    slip_plane_normal_rotated = _slip_plane_normalboth[_qp][i]; //_slip_plane_normal[i];
    _tau_b_local[i] = _initial_thermal + Stress_internal.contract(libMesh::outer_product(
...
DiscoFluxCPOrowanStressUpdate::getDisloVelocity()
{
  ...
  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    ...
    tau_eff[i] = (_tau[_qp][i] - _tau_b_local[i] - _slip_thermal[_qp][i]); // this value is changed
    tau_effAbs[i] = std::abs(_tau[_qp][i]) - _tau_b_local[i] - _slip_thermal[_qp][i];
