...
#include "AdvectionSUPGDisloc.h"
#include "Function.h"

registerMooseObject("discopanterApp", AdvectionSUPGDisloc);

InputParameters
AdvectionSUPGDisloc::validParams()
{
...
AdvectionSUPGDisloc::AdvectionSUPGDisloc(const InputParameters & parameters)
  : INSBase(parameters),
...
Real
AdvectionSUPGDisloc::computeQpResidual()
{
...
Real
AdvectionSUPGDisloc::computeQpJacobian()
{
  Real tau_val = (_tau_type == "opt" ? tauNodal() : tau());
  RealVectorValue U(_u_vel[_qp], _v_vel[_qp], _w_vel[_qp]);
