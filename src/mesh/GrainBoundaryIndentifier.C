...
GrainBoundaryIndentifier::validParams()
{
  ...
  params.addRequiredParam<std::vector<BoundaryName>>("new_boundary", "Grain boundary name");
  params.addClassDescription("Class that identifies grain broudaries between subdomains.");

  return params;
}

GrainBoundaryIndentifier::GrainBoundaryIndentifier(const InputParameters & parameters)
    ...
    for (unsigned int side = 0; side < elem->n_sides(); side++)
    {
      ...
      if (neighbor == remote_elem)
      {
        queries[elem->processor_id()].push_back(std::make_pair(elem->id(), side));
               ...
               elem->subdomain_id() != neighbor->subdomain_id())
